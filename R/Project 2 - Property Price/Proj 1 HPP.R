...
for (i in 1:ncol(dn)){
  ...
  #ggplot(dn,aes(i,SP))+geom_point()
  
 plot(dn[[i]],SP,xlab=colnames(dn[[i]]))
...
#dfcat$Pool_Quality[which(is.na(dfcat$Pool_Quality))] <- 'None' (not working have to add levels)
#dfcat$Pool_Quality[dfcat$Pool_Quality=='<NA>'] <- 'None'
# Get levels and add "None"
# levels <- levels(dfcat$Pool_Quality)
# levels[length(levels) + 1] <- "None"
# levels(dfcat$Pool_Quality)<-c(levels(dfcat$poPool_Quality),"<NA>")  #Add the extra level to your factor
# dfcat$Pool_Quality[is.na(dfcat$Pool_Quality)] <- "None"  
# dfcat$Pool_Quality <- ifelse(is.na(dfcat$Pool_Quality), 'None', dfcat$Pool_Quality)
# dfcat$Fence_Quality <- ifelse(is.na(dfcat$Fence_Quality), 'None', dfcat$Fence_Quality)
# dfcat$Miscellaneous_Feature <- ifelse(is.na(dfcat$Miscellaneous_Feature), 'None', dfcat$Miscellaneous_Feature)
...
head(data.frame(train$Sale_Price, pred))
sqrt(mean((train$Sale_Price-pred)^2))
